...
void Adafruit_PWMServoDriver::setServo(uint8_t ch,uint8_t angle){
  int angle_servo = map(angle,0,180,100,475);
  setPWM(ch, 0, angle_servo);
}
void Adafruit_PWMServoDriver::setServoByStep(uint8_t ch,uint8_t from_angle,uint8_t traget_angle,uint8_t time){
  int angle_servo = map(from_angle,0,180,100,475);
  int angle_servo_traget = map(traget_angle,0,180,100,475);
  if(angle_servo_traget > angle_servo){
    for(int i = angle_servo;i<angle_servo_traget;i++){
      setPWM(ch, 0, i);
      delay(time);
    }
  }
